...
export interface DeploymentContextMenuProps {
  deployment: DeploymentItem | null;
  position: { x: number; y: number } | null;
  onClose: () => void;
  onInspectDetails: (item: DeploymentItem, tab?: "yaml" | "spec" | "events") => void;
  onShowPods: (workloadName: string, namespace: string) => void;
  onShowEvents: (item: DeploymentItem) => void;
  onCopyName: (item: DeploymentItem) => void;
  onCopySelector: (item: DeploymentItem) => void;
  onRestartRollout: (item: DeploymentItem) => void;
  onScaleReplicas: (item: DeploymentItem) => void;
  onDelete: (item: DeploymentItem) => void;
  onViewLogs?: (item: DeploymentItem) => void;
  mutationsDisabled?: boolean;
}

export const DeploymentContextMenu: React.FC<DeploymentContextMenuProps> = ({
  deployment,
  position,
  onClose,
  onInspectDetails,
  ...
  if (!position || !deployment) return null;
  ...
  return createPortal(
    ...
    >
      ...
      <div className="px-3 py-1 text-[11px] font-mono border-b border-zinc-800/80 mb-1 flex items-center justify-between">
        <span className="text-zinc-200 font-semibold truncate mr-2" title={deployment.name}>
          {deployment.name}
        </span>
        <span
      ...
      <button
        ...
        onClick={() => {
          onClose();
          onInspectDetails(deployment, "yaml");
        }}
      ...
      <button
        ...
        onClick={() => {
          onClose();
          onShowPods(deployment.name, deployment.namespace);
        }}
        className="w-full px-3 py-1.5 text-left hover:bg-zinc-800/80 hover:text-white flex items-center space-x-2 transition-colors cursor-pointer"
      >
      ...
      <button
        ...
        onClick={() => {
          onClose();
          onCopyName(deployment);
        }}
        className="w-full px-3 py-1.5 text-left hover:bg-zinc-800/80 hover:text-white flex items-center space-x-2 transition-colors cursor-pointer"
      >
        <Copy className="w-3.5 h-3.5 text-zinc-400" />
        <span>Copy Deployment Name</span>
      </button>
      ...
      <button
        ...
        onClick={() => {
          onClose();
          onCopySelector(deployment);
        }}
        className="w-full px-3 py-1.5 text-left hover:bg-zinc-800/80 hover:text-white flex items-center space-x-2 transition-colors cursor-pointer"
      >
        <Copy className="w-3.5 h-3.5 text-zinc-400" />
        <span>Copy Selector Labels</span>
      </button>
